...
func (r *ServiceRepository) Update(ctx context.Context, id uuid.UUID, input *domain.UpdateServiceInput, updatedBy uuid.UUID) error {
	...
	if input.CategoryID != nil {
		updates["category"] = input.CategoryID.String()
	}
...
// ListByBusiness retrieves services by business ID
func (r *ServiceRepository) ListByBusiness(ctx context.Context, businessID uuid.UUID, page, pageSize int) ([]*domain.Service, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to list services by business: %w", err)
	}
...
func (r *ServiceRepository) ListByCategory(ctx context.Context, categoryID uuid.UUID, page, pageSize int) ([]*domain.Service, error) {
	...
	err := r.WithContext(ctx).
		Preload("Business").
		Where("category = ? AND is_active = ?", categoryID.String(), true).
		Offset(offset).
		Limit(pageSize).
		Order("created_at DESC").
		Find(&serviceModels).Error
...
// CountByBusiness counts services by business ID
func (r *ServiceRepository) CountByBusiness(ctx context.Context, businessID uuid.UUID) (int64, error) {
	var count int64

	err := r.WithContext(ctx).Model(&models.Service{}).Where("business_id = ?", businessID).Count(&count).Error
	if err != nil {
		return 0, fmt.Errorf("failed to count services by business: %w", err)
	}
...
func mapServiceDomainToModel(s *domain.Service) *models.Service {
	...
	// Handle CategoryID - convert to Category string
	if s.CategoryID != nil {
		serviceModel.Category = s.CategoryID.String()
	}
...
func mapServiceModelToDomain(s *models.Service) *domain.Service {
	...
	// Handle Category string - convert to CategoryID UUID
	if s.Category != "" {
		if categoryID, err := uuid.Parse(s.Category); err == nil {
	...
	if s.Business.ID != uuid.Nil {
		service.Business = mapBusinessModelToDomain(&s.Business)
	}

	// Note: Category is a string field in the model, not a relationship
