...
func TestStaffPerformanceModel(t *testing.T) {
	...
	staffID := uuid.New()
	joinDate := time.Now().Add(-180 * 24 * time.Hour) // 180 days ago
	staff := models.Staff{
		BaseModel: models.BaseModel{
			ID: staffID,
		},
	...
	// Create a monthly performance record
	...
	var firstDayOfMonth time.Time
	var lastDayOfMonth time.Time

	if currentMonth == 1 {
		firstDayOfMonth = time.Date(currentYear-1, 12, 1, 0, 0, 0, 0, currentLocation)
		lastDayOfMonth = time.Date(currentYear, 1, 1, 0, 0, 0, 0, currentLocation).Add(-time.Nanosecond)
	} else {
		firstDayOfMonth = time.Date(currentYear, currentMonth-1, 1, 0, 0, 0, 0, currentLocation)
		lastDayOfMonth = time.Date(currentYear, currentMonth, 1, 0, 0, 0, 0, currentLocation).Add(-time.Nanosecond)
	}
